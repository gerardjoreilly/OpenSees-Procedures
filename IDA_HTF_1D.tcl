# --------------------------------------------------------------------------------------------------
# -- Script to Conduct 1D Incremental Dynamic Analysis Using Hunt, Trace and Fill Algorithm -----------
# --------------------------------------------------------------------------------------------------
# Gerard O'Reilly
# EUCENTRE/IUSSPavia
...
# runNRHA1D: This is to run the actual non-linear response history analysis
# getSaT: Get the spectral response ordinates
# Arias: Get the Arias intensity of a record
#
proc IDA_HTF_1D {firstInt incrStep maxRuns IMtype Tinfo xi Dt dCap gmsdir nmsfile dtsfile dursfile outsdir mdlfile {pflag 0}} {
	...
	set eqnms_list 	[read [open $gmsdir/$nmsfile "r"]];
	...
	for {set i 1} {$i <= $nrecs} {incr i 1} {
		...
		set EQname [lindex $eqnms_list $i-1]; 	# Get the name of the record
		...
		if {$IMtype==1} {
			...
			getSaT $gmsdir/$EQname $dt 1.0 $xi; # Get the PGA of the record in the X direction
			set IMrecord $pga
			...
			# Now get the spectral ordinates
			getSaT $gmsdir/$EQname $dt $Tcond $xi; # Get the Sa(T1,5%) of the record in the X direction
			set IMrecord $Sa
		...
		while {$j<=$maxRuns} {
			...
			if {$hFlag==1} {
				...
				set sf 	[expr [lindex $IM $j-1]/$IMrecord*$g];
				...
				runNRHA1D $Dt $dur $dCap $tNode $bNode $log $pflag
				close $log
				incr j 1;
